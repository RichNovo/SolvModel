...
def calc_distance(
    ...
    rot_diff = None
    if nodes_rot is not None:
        neigh_abs_pos = nodes_rot[edges[:, 0]]  # num_edges, 3
        this_pos = nodes_rot[edges[:, 1]]  # num_edges, 3
        rot_diff = this_pos - neigh_abs_pos  # num_edges, 3

    if return_diff:
        return dist, diff, rot_diff
    else:
        return dist, rot_diff
...
def rot_expansion(input_x: torch.Tensor, expand_params: List[Tuple]):
    feat_list_x = input_x[:,0].unsqueeze(-1)
    feat_list_y = input_x[:,1].unsqueeze(-1)
    feat_list_z = input_x[:,2].unsqueeze(-1)

    feat_list_x = torch.nan_to_num(sinc_expansion(feat_list_x, expand_params),0)
    feat_list_y = torch.nan_to_num(sinc_expansion(feat_list_y, expand_params),0)
    feat_list_z =torch.nan_to_num(sinc_expansion(feat_list_z, expand_params),0)

    return [feat_list_x,feat_list_y,feat_list_z]
...
class PaiNNInteraction(nn.Module):
    ...
    def __init__(self, node_size, edge_size, cutoff, timestep_embedding = False):
        """
        Args:
            node_size (int): Size of node state
            edge_size (int): Size of edge state
        ...
        #self.filter_rot_layer_x = nn.Linear(edge_size, 3 * node_size, bias=False)
        #self.filter_rot_layer_y = nn.Linear(edge_size, 3 * node_size, bias=False)
        #self.filter_rot_layer_z = nn.Linear(edge_size, 3 * node_size, bias=False)

        if False:
            self.filter_rot = nn.Linear(3 , 3 * node_size, bias=False)

            self.rot_length_reducer = nn.Linear(2 * 3 * node_size , 3 * node_size, bias=False)
    ...
    def forward(
        ...
        if False:#self.timestep_embedding:
            #if rot_edge_state is not None:
                #filter_rot_weight_x = self.filter_rot_layer_x(rot_edge_state[0])  # num_edges, 3*node_size
                #filter_rot_weight_y = self.filter_rot_layer_y(rot_edge_state[1])  # num_edges, 3*node_size
                #filter_rot_weight_z = self.filter_rot_layer_z(rot_edge_state[2])  # num_edges, 3*node_size
            filter_rot_weight = self.filter_rot(timestep_embedding_state) #(filter_rot_weight_x+filter_rot_weight_y+filter_rot_weight_z)/3.0
                #filter_weight = filter_rot_weight*filter_weight
            filter_weight = self.rot_length_reducer(torch.cat([filter_weight,filter_rot_weight], dim = -1))
