...
class TransformAtomsObjectsToGraphXyz:
    ...
    def __call__(self, atoms):
        ...
        # pylint: disable=E1102
        graph_data = {
            "nodes": atoms.get_atomic_numbers(),#torch.tensor(atoms.get_atomic_numbers()),
            "nodes_xyz": atoms.get_positions(),#torch.tensor(atoms.get_positions(), dtype=default_type),
            "num_nodes": torch.tensor(len(atoms.get_atomic_numbers())),
            "edges": edges,
            "edges_displacement": edges_displacement,
            "cell": torch.tensor(np.array(atoms.get_cell()),dtype=atoms.get_positions().dtype),
            "num_edges": torch.tensor(edges.shape[0])
        }


        if hasattr(atoms, 'get_rot_list'):
            rot_list = atoms.get_rot_list()
            if len(rot_list)>0 and rot_list[0] is not None:
                graph_data["nodes_rot"] = rot_list
        if hasattr(atoms, 'get_timestep_list'):
            timestep_list = atoms.get_timestep_list()
            if len(timestep_list)>0 and timestep_list[0] is not None:
                graph_data["timestep"] = timestep_list
    ...
    def get_edges_simple(self, atoms):
        ...
        valid_indices_bool = dist_mat < self.cutoff
        #np.fill_diagonal(valid_indices_bool, False)  # Remove self-loops
        valid_indices_bool.fill_diagonal_(False) # Remove self-loops
        #edges = np.argwhere(valid_indices_bool)  # num_edges x 2
        edges = torch.argwhere(valid_indices_bool)
        #edges_displacement = np.zeros((edges.shape[0], 3))
        edges_displacement = torch.zeros((edges.shape[0], 3))
